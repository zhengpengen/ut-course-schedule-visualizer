...
const ClassGroup = () => {
  ...
  const addGroupCard = () => {
    setGroupCards([...groupCards, nextId]);
  ...
  const handleClickOutside = (event) => {
    if (helpScreenRef.current && !helpScreenRef.current.contains(event.target)) {
    ...
    <div className={`class-group ${showHelpScreen ? 'blur' : ''}`}>
      <div className="content-wrapper">
        ...
        <div className="main-content d-flex align-items-center">
          {groupCards.map(id => (
            <GroupCard key={id} groupNumber={id} onDelete={() => deleteGroupCard(id)} onCountChange={(count) => updateGroupCount(id, count)} />
          ))}
